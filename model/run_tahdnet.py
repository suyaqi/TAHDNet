...
def load_dataset(args):
    ...
    # load data
    # print(os.path.join(data_dir, 'data-multi-visit.pkl'))
    # print(os.path.join(data_dir, 'data-single-visit.pkl'))
    # d_dir = data_dir + '/data-multi-visit.pkl'
    # data_multi = pd.read_pickle(d_dir).iloc[:, :4]
    data_multi = pd.read_pickle(os.path.join(
        data_dir, 'data-multi-visit.pkl')).iloc[:, :4] # [0,1,2,3,5]SUBJ_ID, HADM_ID, ICD9, ATC4, STARTDATE
    data_single = pd.read_pickle(
        os.path.join(data_dir, 'data-single-visit.pkl')) # SUBJ_ID, HADM_ID, ICD9, ATC4, STARTDATE
...
def main():
    ...
    # Required parameters
    parser.add_argument("--model_name", default='TAHDNet-pretraining010', type=str, required=False,
                        help="model name")
    ...
    parser.add_argument("--pretrain_dir", default='../saved/TAHDNet-predict', type=str, required=False,
    ...
    parser.add_argument("--use_pretrain",
                        default=False,
                        action='store_true', # 只要加上这个，--use_pretrain无论传参如何都置为true，如果不传此参数则为default
    ...
    os.makedirs(args.output_dir, exist_ok=True) # 创建多层目录
    # exist_ok=True存在该目录时不触发FileExistsError异常，但也不创建新的 无事发生
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for _, batch in enumerate(prog_iter):
                ...
                # print('dx_labels', dx_labels.shape)
                # print('rx_labels', rx_labels.shape)
    ...
    if args.do_test:
        ...
        def test(task=0):
            ...
            model_state_dict = torch.load(dx_output_model_file)
            ...
            y_preds = []
            y_trues = []
            for test_input in train_dataloader:
                test_input = tuple(t.to(device) for t in test_input)
                input_ids, dx_labels, rx_labels, input_interv = test_input
                input_ids, dx_labels, rx_labels, input_interv = input_ids.squeeze(
                    dim=0), dx_labels.squeeze(dim=0), rx_labels.squeeze(dim=0), input_interv.squeeze(dim=0)
                with torch.no_grad():
                    loss, rx_logits = model(
                        input_ids, input_interv, dx_labels=dx_labels, rx_labels=rx_labels)
                    # med_pred = rx_logits.clone().detach().cpu().numpy()[0]
                    # med_pred[med_pred >= 0.5] = 1
                    # med_pred[med_pred < 0.5] = 0
                    # idx_pred = list(np.where(med_pred == 1)[0])
                    # print('med', med_pred)
                    # med_pred = EHRTokenizer(args.data_dir).convert_ids_to_tokens(idx_pred)
                    # print('rx_logits', rx_logits)

                    # print('idx_pred', idx_pred)
                    # print('med_pred', med_pred)
