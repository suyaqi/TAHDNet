...
from utils import metric_report, t2n, get_n_params, multi_label_metric
        ...
        def transform_data(data):
            ...
            for subject_id in data['SUBJECT_ID'].unique():
                ...
                for _, row in item_df.iterrows():
                    admission = [list(row['ICD9_CODE']), list(row['ATC4']), list(row['interval'])]
    ...
    def __getitem__(self, item):
        ...
        for idx, adm in enumerate(self.records[subject_id]):
            ...
            if idx != 0:
                ...
                inp_interval.append(list(adm[2]))
        ...
        if cur_id < 5:
            ...
            logger.info("input_interval: %s" %
                        " ".join([str(x) for x in inp_interval]))
        ...
        cur_tensors = (torch.tensor(input_ids).view(-1, self.seq_len),
                       ...
                       torch.tensor(inp_interval))
        # print(cur_tensors[0].size(), cur_tensors[1].size(), cur_tensors[2].size())
        return cur_tensors
...
def main():
    ...
    parser.add_argument("--pretrain_dir", default='../saved/GBert-pretraining123', type=str, required=False,
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for _, batch in enumerate(train_dataloader):
                ...
                # print('batch', batch)
                input_ids, dx_labels, rx_labels, input_interv = batch
                # print("\ninput size", input_ids.size(), dx_labels.size(), rx_labels.size(),input_interv.size())
                # print(input_ids)_
                ...
                # print("mode_input size", input_ids.size(), dx_labels.size(), rx_labels.size(), input_interv.size())
                loss, rx_logits = model(input_ids, input_interv, dx_labels=dx_labels, rx_labels=rx_labels,
                                        epoch=global_step)
                loss.backward()
            ...
            if args.do_eval:
                ...
                for eval_input in tqdm(eval_dataloader, desc="Evaluating"):
                    ...
                    input_ids, dx_labels, rx_labels, input_interv = eval_input
                    input_ids, dx_labels, rx_labels, input_interv = input_ids.squeeze(
                        dim=0), dx_labels.squeeze(dim=0), rx_labels.squeeze(dim=0), input_interv.squeeze(dim=0)
                    with torch.no_grad():
